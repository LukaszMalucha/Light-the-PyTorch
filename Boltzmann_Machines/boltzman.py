...
# total number of users/movies by choosing maxiumu value for the column in train/test set
nb_users = int(max(max(training_set[:,0]), max(test_set[:,0])))   ## max might be either in test or training set. Convert to int to avoid an error
nb_movies = int(max(max(training_set[:,1]), max(test_set[:,1])))  ## max might be either in test or training set. Convert to int to avoid an error

# Matrix with users as rows and movies as columns(list of lists for Torch) 943x1682
## List of every user ratings with Movie Rating or Zero if never rated
def convert(data):
    new_data = []
...
## List of lists expected for tensor conversion
